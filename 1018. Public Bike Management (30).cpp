...
Sample Output:
3 0->2->3 0
费用不满足最优子结构性质，需要在用dijkstra保存所有可能的路径，并在此基础上运用dfs解决问题。
*/
#include<iostream>
#include<vector>
#include<algorithm>
...
int e[maxv][maxv],dist[maxv],nv,ve,s,d,cap,weight[maxv],minback=inf,minneed=inf;
vector<int> tmpath,pre[maxv],path;
bool visit[maxv];
int find_min_dist() {
    int min=inf,index=-1;
    for(int i=0;i<nv;++i) {
...
void dfs(int v) {
    if(v==s) {
        ...
        //int need=(tmpath.size()-1)*cap-tmptotal;
        if(need<minneed) {
            minneed=need;
            minback=back;
            path=tmpath;
